...
Starting or cancelling a PDF export needs **editor**. Listing exports and downloading a PDF need only **viewer**.

## Rate Limiting

Each API key has a per-minute request limit configured at creation time, counted across every endpoint. Starting and cancelling PDF exports has an additional limit of 10 requests a minute per key.
...
Poll it until `status` reaches `completed`, `failed`, or `cancelled`. `progress` counts pages as they render:

```bash
curl https://api.documentationai.app/api/v1/exports/pdf/6fa5a52d-d8b7-4b24-ac92-a55378f5eec4 \
  -H "Authorization: Bearer dai_your_api_key"
```

When `downloadable` is `true`, download the file:

```bash
curl -o documentation.pdf \
  https://api.documentationai.app/api/v1/exports/pdf/6fa5a52d-d8b7-4b24-ac92-a55378f5eec4/download \
  -H "Authorization: Bearer dai_your_api_key"
```
...
- One export runs per documentation at a time. Starting another returns `409 Conflict`, and the message names the export in progress.
...
- A PDF stays available until a newer export of the same documentation completes and replaces it. Earlier exports stay in `GET /exports/pdf`, with `downloadable` set to `false`.
- `DELETE /exports/pdf/{exportId}` cancels an export that is still running, or deletes the file of a finished one.
