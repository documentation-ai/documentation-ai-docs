...
When your OpenAPI specification defines multiple servers or uses server variables, the playground automatically renders a server selector. Readers can switch between environments (for example, production, staging, or a regional endpoint) and fill in server variables directly from the UI.
...
servers:
  ...
  - url: https://{env}.api.example.com/v2
...
- **Lists all defined servers** from the OpenAPI `servers` array, showing the description and resolved URL for each.

- **Resolves server variables** — when a server URL contains `{variable}` placeholders, the playground shows input fields so readers can substitute values. The resolved URL updates in real time.

- **Persists across sessions** — the selected server and variable values are saved in the browser's `localStorage`, so readers don't need to reconfigure on every visit.

- **Updates the request preview** — the generated code snippets and request preview always reflect the currently selected server URL.

### Parameter Example Pickers

When an OpenAPI parameter defines multiple `examples`, the playground renders a dropdown picker instead of a plain text input. Readers can select from the provided examples or type a custom value, making it faster to send realistic test requests without looking up valid values.
