...
| ---------- | ----------------------------------------------------------- | ---------------------------------------------------------------------- |
| **Viewer** | All read endpoints, including analytics, media, and storage | None                                                                   |
| **Editor** | All read endpoints                                          | Push, branches, merge, deploy, PDF exports, and media except deletion  |
| **Admin**  | All read endpoints                                          | All write endpoints, including deleting media                          |
...
Upload images, videos, and files to your media library, then use their URLs in your pages. Every file gets the same checks as an upload in the web editor: a supported type, your plan's size limit, and your storage quota. See [Limits and supported file types](/write-and-publish/media-library#limits-and-supported-file-types).

Upload up to 10 files in one `multipart/form-data` request:
...
Or import up to 10 files from public HTTPS URLs:
...
curl -X POST https://api.documentationai.app/api/v1/media/import \
  ...
  -d '{
    "files": [
      { "url": "https://example.com/setup-diagram.png", "altText": "Setup steps diagram" }
    ]
  }'
```

Both return one result per file, so one failed file does not fail the others:

```json
{
  "results": [
    {
      "source": "https://example.com/setup-diagram.png",
      "status": "added",
      "media": {
        "id": "4c1d8a2e-7b3f-4f0a-9d6e-2a5b8c7e1f03",
        "name": "setup-diagram.png",
        "url": "https://blob-cdn.documentation.ai/.../setup-diagram.png",
        "kind": "image",
        "mimeType": "image/png",
        "sizeBytes": 184233,
        "altText": "Setup steps diagram",
        "width": 1440,
        "height": 900,
        "snippet": "<Image src=\"https://blob-cdn.documentation.ai/.../setup-diagram.png\" alt=\"Setup steps diagram\" />"
      }
    }
  ]
}
...
`status` is `added` for a new file, `reused` when the same file is already in the library, or `failed` with an `error` that says why. If another file already has the same name, the new file gets a number added, such as `setup-diagram-2.png`. Paste `snippet` into a page to show the file.

The response code is `200` when every file was stored and `207` when only some were. When every file fails, it is the status of the first failure, such as `413` for a file over your plan's size limit.

Your organization can store up to 120 files a minute through the API. Past that, a file fails with status `429` and an error that says how many seconds to wait.

To swap in a newer version without changing its URL, send the new file, or a `url`, to `POST /media/{mediaId}/replace`. The new file must be the same type, and every page that uses the file shows the new version.

Deleting files needs an **admin** key. Use `DELETE /media/{mediaId}` for one file, or `POST /media/bulk-delete` for up to 100. Pages that use a deleted file show it as missing.

Check your storage before a large upload:
...
`usedBytes` and `quotaBytes` cover every documentation in your organization. `limits` gives the largest file your plan accepts for each kind of file.
## PDF Exports

Export your live site as a single PDF, for offline reading, compliance archives, or sharing outside your docs. Pages are rendered in navigation order. PDF export is available on the Standard, Professional, and Enterprise plans.
