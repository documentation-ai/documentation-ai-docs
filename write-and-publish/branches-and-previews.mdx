...
## When a branch is unavailable

If the editor opens on a branch that no longer exists (deleted, renamed, or inaccessible), a recovery screen appears instead of a blank page. The screen lists available branches on the repository, with the deployment branch shown first and marked with a **Live** badge. Use **Check again** to reload the branch list, then select a branch to continue working.
...
This setting lives in Git Settings, not in the branch menu in the editor.

<Steps>
  <Step title="Open your Git Settings" icon="settings" title-type="p">
    Go to **Settings → Git Settings**, then open the settings for your connected repository (GitHub or GitLab).
  </Step>
...
For file-based Git workflows, see the [Code Editor](/write-and-publish/code-editor). To change which branch is treated as Live, update the Live branch setting in **Settings → Git Settings**.
