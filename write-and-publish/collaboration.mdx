...
description: "Edit documentation together in real time with inline comments, mentions, and notifications directly in the web editor."
---
...
<Steps>
  ...
  <Step title="Create a comment thread" icon="edit">
...
After you post the comment, the selected text stays highlighted and quoted in the thread. That highlight confirms the thread is attached to the correct text.
  ...
  <Step title="Post and manage replies" icon="message-circle">
...
Use replies to keep discussion tied to one piece of text instead of scattering feedback across separate comments.
  ...
  <Step title="Resolve or reopen a thread" icon="check-circle">
...
Resolved threads stay available for reference, which helps teams track why a decision was made.
  ...
  <Step title="Browse all threads in the Comments panel" icon="folder">
...
The editor URL includes the active branch name, such as `/editor/main` or `/editor/feature-xyz`. Use the branch selector in the navbar to switch branches without leaving the editor.

Browser back and forward navigation also works between branches, which makes it easier to compare work across active efforts. Each branch keeps its own collaborative session, so collaborators on different branches do not see each other's edits, comments, or presence.
...
<Steps>
  ...
  <Step title="Open the Save menu" icon="chevron-right">

Click the Save dropdown in the editor navbar to see the available publishing actions for the current branch.


  </Step>
  <Step title="Save changes to the current branch" icon="check-circle">
...
Choose **Save** to commit your current edits to the active branch. Use this option when you want to preserve work without merging it yet.
  ...
  <Step title="Merge into the default branch" icon="git-merge">
...
Choose **Save and Merge** to commit your changes and merge them into the default branch in one step. Use this when the branch is ready to publish without additional review.
  ...
  <Step title="Create a pull request for review" icon="github">
...
<Columns cols={3}>
  ...
  <Card title="Web Editor" href="/write-and-publish/web-editor" icon="edit" cta="Open guide" />

  <Card title="Branches and Previews" href="/write-and-publish/branches-and-previews" icon="folder" cta="Learn branch workflows" />

  <Card title="Team Management" href="/account-and-billing/team-management" icon="users" cta="Manage roles" />
