...
<Update label="2026-08-03" description="Reader MCP Server authentication with OAuth 2.1, MCP credential management, collaborative editing for CSS and Scripts, and critical editor fixes" tags={["feature","improvement","fix"]}>
  ...
  The Reader MCP Server now supports authenticated access to private, partial, and password-protected documentation through OAuth 2.1. When your documentation has access control enabled, MCP clients connect through a full OAuth flow: the client opens a browser, the user approves the connection and signs in through your site's existing authentication, and an MCP credential is minted automatically. Public sites continue to use the existing `/_mcp` endpoint. Access-controlled sites use the new `/_mcp/auth` endpoint.
  ...
  Manage credentials for the Reader MCP Server directly from the dashboard. Create machine-to-machine credentials with a name, access roles, and expiry. View all credentials in a table, edit access roles on existing credentials without re-issuing them, and revoke individual credentials or all at once. Newly created tokens are shown once with a copy button for the token and the ready-made client configuration JSON.
