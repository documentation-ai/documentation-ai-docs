...
<Update label="2026-08-18" description="Broken Link Audit, seat limit increases, and llms.txt auth support" tags={["feature","improvement","fix"]}>
  ...
  - **Style attributes in MDX**: Inline `style="..."` attributes (commonly used in embedded iframes and video embeds) are now correctly converted to JSX-compatible objects at the rendering layer.
...
<Update label="2026-08-11" description="Self-Heal by User Feedback workflow, agent analytics, branch recovery, per-user content, and critical fixes" tags={["feature","improvement","fix"]}>
  ...
  - **Search indexing failure on API reference pages**: Double quotes in OpenAPI descriptions broke JSX syntax, causing HTTP 500 on API reference pages.
