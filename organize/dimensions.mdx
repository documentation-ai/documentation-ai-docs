...
See [Site Configuration](/customize/site-configuration) for file-based management.
...
## Managing Dimensions in documentation.json

Configure Dimensions directly in `documentation.json`. This is useful when you want:

- Making changes using Code editor 
- Version-controlled navigation changes
- Bulk edits across many dimension branches

### Standalone product dimension

Define a `products` array at the root of your navigation. Each product becomes a branch with its own tabs, groups, and pages:
...
### Standalone version dimension

Use a `versions` array when a single product has multiple API or release versions:
...
### Nested dimensions (Product → Version)

Nest dimension arrays to combine multiple dimensions. The outer dimension wraps the inner:
...
For language dimensions, see [Localization](/organize/localization#configuring-languages-in-documentationjson) for full JSON examples including nested version + language structures.

See [Site Configuration](/customize/site-configuration) for the full configuration reference.

<Callout kind="info" collapsed="false">
  The `search.strict-dimensions` setting in `documentation.json` controls whether the AI assistant searches across all dimensions or only within the reader's current product, version, and language. See [strict dimensions](/ai/ai-assistant#strict-dimensions) for full details.
</Callout>
...
Products, versions, and languages are structural containers by default; they define context but do not have pages of their own. You can attach a **content page** to any dimension value.
...
#### Attach a content page in the Web Editor

1. In the editor sidebar, click the **menu icon** next to the dimension value name.
2. Select **Attach Content Page**.
3. Enter a path (slug) for the page, or browse unpublished pages to reuse an existing MDX file.
4. The dimension value now opens its content page when clicked.

To remove the content page later, use **Detach Content Page**. The MDX file remains as an unpublished page.

#### Attach a content page in configuration

Add a `path` property to the dimension entry in `documentation.json`:
...
The `path` follows the same rules as any page path: lowercase kebab-case, unique across your docs, and without the `.mdx` extension. To remove the content page, delete the `path` property.

#### How content pages affect navigation

When a dimension value has a content page:

- The dimension label becomes a **clickable link** in breadcrumb navigation.
- The content page is included in **search results** and **AI features**.
- Display options like `badge`, `show-sidebar`, and `content-width` work the same as on regular pages.
...
- [Organize content and navigation in the Web Editor](https://documentation.ai/docs/write-and-publish/web-editor#organizing-structure-in-the-web-editor) for editor-based navigation management.
