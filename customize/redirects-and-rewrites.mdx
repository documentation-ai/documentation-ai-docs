...
If your documentation is deployed at a subpath, such as `example.com/docs`, redirects are normalized against the configured subpath. Write `source` and `destination` paths as documentation-relative paths, without the subpath prefix:
...
- A redirect written as `/docs/old-page` → `/docs/new-page` is normalized and will not produce duplicated prefixes like `/docs/docs/new-page`

Subpath matching uses path boundaries. A configured subpath of `/docs` applies to `/docs` and `/docs/old-page`, but not to unrelated paths like `/docs-preview`.
