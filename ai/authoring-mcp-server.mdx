...
The Authoring MCP Server and the Content API cover largely the same documentation-management tasks, but they expose them through different interfaces. Choose the Authoring MCP Server when you want an MCP-compatible AI client or agent to do the work for you, and choose the Content API when you want direct REST API access from your own scripts, services, or CI/CD. For endpoint details, see the [API reference overview](/api-reference/overview).

<Callout kind="info">

Use this server for authoring and documentation operations through an MCP client. If you need direct REST API access instead, use the Content API. If your AI tool only needs read-only access to published content, use the [Docs MCP Server](/ai/docs-mcp-server).
