...
<Update label="2026-06-30" description="PDF export, SVG and HTML Block components, SEO meta overrides, API playground server management, heading copy-link, and platform improvements" tags={["feature","improvement","fix"]}>
  ...
  Clicking any heading on a published page now copies its anchor link to the clipboard and updates the browser URL hash. A brief check-icon animation confirms the copy, making it easy to share deep links to specific sections.

  ## External Link Auto-Target

  External links in your documentation now automatically open in a new browser tab with `rel="noopener noreferrer"` for security. Internal links and hash anchors are unaffected, and explicit `target` overrides are respected.

  ## Improvements

  - **AI Agent Architecture** — Dashboard and background agents are now unified, using a streamlined pipeline and a more cost-efficient model. Background workflows benefit from improved file operations, PR merge context, and bot commit detection to avoid re-triggering agent-authored changes.
  ...
  - **Plan Limits** — Standard plan seats increased from 2 to 3, Professional plan seats from 5 to 10, and Standard plan documentation sites from 1 to 2.
  ...
  - **Configurable Root Domain** — The docs base domain can now be set via the `NEXT_PUBLIC_DOCS_BASE_DOMAIN` environment variable, enabling white-label deployments without code changes.

  ## Fixes

  - **Branch Slug DNS Compliance** — Preview branch subdomains are now enforced to stay within the 63-character DNS label limit, with trailing dash cleanup and collision-safe hashing.

  - **SVG Rendering Safety** — SVG injection now uses DOM parsing (`DOMParser`) instead of `innerHTML`, with error detection for malformed markup.

  - **Documentation Name Uniqueness** — Removed the restriction that blocked creating a documentation project with the same name as an existing one in your organization.

  - **Disabled Workflow Templates** — Temporarily disabled workflow templates now show an informative tooltip explaining they will be re-enabled in a future release.
