...
The credit cost per question depends on the assistant mode configured for your documentation site:

| Mode  | Credits per question | Best for                                      |
| ----- | -------------------- | --------------------------------------------- |
| **Pro**  | 10                   | In-depth answers for detailed and multi-step questions |
| **Lite** | 1                    | Quick, focused answers for everyday questions        |

For example, on the Standard plan (10,000 credits) with Pro mode, your users can ask up to 1,000 questions per month. With Lite mode, the same allocation supports up to 10,000 questions.

Eligible organizations can switch between Pro and Lite from the AI Assistant settings page. The selected mode is stored per documentation site. See [AI Assistant](/ai/ai-assistant#assistant-mode) for details on switching modes.

### When credits are not deducted

Credits are not consumed in the following situations:

- **Response failure** — If the assistant encounters an error and cannot generate a response, no credits are deducted.
- **No relevant answer found** — If the assistant searches your documentation but cannot find information relevant to the user's question, no credits are deducted.
- **Casual conversation** — Greetings, small talk, and non-documentation messages (e.g., "Hi," "Thank you") do not consume credits.
...
- **Use the AI Assistant for questions** — If you just need to look up something in your docs, use Ask AI (10 credits) instead of the Documentation Agent.
