...
<Update label="2026-09-08" description="Media Library, Video Upload, AI assistant reliability, reader experience fixes, and access control enforcement" tags={["feature","improvement","fix"]}>
  ...
  - **AI streaming reliability**: Long-running AI responses now send keepalive signals to prevent connection drops by browsers, proxies, and load balancers during periods when the agent is thinking or waiting on a tool.
  - **Faster private documentation pages**: Signed-in readers on sites without role-based page restrictions now receive cached pages, reducing load times for authenticated traffic.
  ...
  - **Role-based access enforcement on deployed sites**: JWT and OAuth role restrictions now correctly reach deployed sites and honor the dashboard access-control switch. Password-protected sites no longer enforce stale role tags, and unauthenticated users no longer see protected navigation alongside the login gate.
</Update>
