...
---
<Update label="2026-07-10" description="Real-time collaboration, inline comments, @mentions and notifications, branch-aware editor URLs, and workflow UX updates" tags={["feature","improvement","fix"]}>

  ## Real-Time Collaboration

  Multiple team members can now edit the same documentation page simultaneously with live cursors and automatic saving. Every keystroke syncs to the server in real time — publish commits the accumulated work. See [Web Editor](/write-and-publish/web-editor).

  - The editor automatically switches to the new real-time mode when your branch has no unpublished changes. If you have pending work, you'll see a one-time upgrade prompt guiding you to publish or revert first.

  - Collaborator avatars in the toolbar show who else is currently in the document.

  - Navigation structure and settings changes sync across all editors instantly — no manual refresh needed.

  ## Inline Comments

  Select any text in the editor and press `⌘⌥M` (or use the comment button in the toolbar) to start a discussion. Comments highlight the selected text and open a thread popover for replies. Threads can be resolved and reopened, and a comments panel lists all open and resolved threads for the current branch.

  You can also hover any paragraph and click the comment icon to quickly start a thread without selecting text first.

  ## @Mentions and Notifications

  Type `@` in a comment to autocomplete teammates in your organization. Mentioned users receive a notification in the new bell icon in the sidebar, along with an optional sound alert when they're actively editing. Clicking a notification jumps straight to the relevant comment.

  ## Branch-Aware Editor URLs

  Editor URLs now include the branch name (e.g., `/editor/main` or `/editor/feat/new-api`), so you can bookmark, share, and navigate between branches directly from the browser. The editor sidebar link always opens your last active branch. See [Branches and Previews](/write-and-publish/branches-and-previews).

  ## Branch Actions in the Editor

  A new Save dropdown in the editor toolbar gives you quick access to branch operations: save to branch, save and merge into your default branch, or save and open a pull request. See [Branches and Previews](/write-and-publish/branches-and-previews).

  ## Improvements

  - **Workflow "View Changes"**: Workflow run rows now have a single "View Changes" button that opens the diff view for that branch, replacing the previous "Open in chat" and "Download report" buttons. See [Workflows](/ai/workflows).

  - **Disabled Workflow Templates Hidden**: Templates that aren't yet available are now hidden from the template picker instead of shown as grayed-out options.

  - **Sidebar Badges**: The Editor item in the sidebar now shows a "New" badge, and the "Beta" label has been removed from Workflows.

  - **Toast Notifications**: Toast close buttons are repositioned for easier access, and toasts now include a close button app-wide.

  - **Compact Sidebar Footer**: The user profile area in the sidebar footer is more compact to accommodate the new notification bell.

  ## Bug Fixes

  - **AI Chat Attachments**: Canceling a file upload mid-stream now properly stops the upload and cleans up. Clearing a chat session no longer drops pending attachments.

  - **Session Token Reliability**: API requests that fail due to an expired session token now automatically refresh and retry, reducing spurious authentication errors during long editing sessions.
