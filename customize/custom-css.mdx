---
title: Custom CSS
description: Add custom stylesheets to override default styles, target specific components, and fine-tune your site's visual design.
---
...
Each item in the `customCss` array is an object with one field. A plain string instead of an object is ignored.
...
- Maximum file size is **1 MB** per file
- The file must exist in your documentation project when you publish
- External URLs should use **HTTPS**, because browsers block HTTP stylesheets on HTTPS sites

A project stylesheet that is missing, larger than 1 MB, or has an invalid path does not fail your deployment. It is skipped, and the rest of your site publishes as usual.
...
3. **Custom CSS stylesheets** from your `customCss` array, in the order listed
...
<ExpandableGroup>
  ...
  <Expandable title="A stylesheet from the project is ignored">

  Stylesheets that break the [file requirements](#file-requirements) are skipped when your site deploys. Check that the entry is an object with `src`, that the path is relative and ends in `.css`, that the file exists on the branch you published, and that it is 1 MB or smaller.
