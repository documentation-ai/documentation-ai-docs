...
### Merge progress and workspace reload

When a collaborator initiates a merge, other editors on the same branch see a toast notification showing who is merging and which branch is being merged. The editor enters a read-only state during the merge, and all action buttons (Save, Save and Merge, Save and Create PR) are disabled to prevent conflicting edits.

Once the merge completes, each collaborator's workspace reloads automatically with the merged content. The editor returns to its editable state and a toast confirms the update.
