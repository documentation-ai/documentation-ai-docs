...
description: Create reusable MDX and JSX snippets once, insert them across pages, and keep shared documentation synchronized through the web or code editor.
---
...
Snippets are repository files stored in the top-level `snippets/` directory. Documentation.AI supports Markdown snippets in `.md` and `.mdx` files, plus React component snippets in `.jsx` files.
...
<CodeGroup tabs="MDX snippet,JSX snippet">
...
JSX snippets are React components stored in `.jsx` files. Documentation.AI compiles them in the browser and renders the component in the snippet preview. React hooks, component state, and browser APIs are supported.

A JSX snippet can export a component with a name that matches the filename-derived component name. For example, `status-card.jsx` can contain:
...
JSX snippets do not support imports from external packages. Use React features and browser APIs available in the snippet environment, but do not import arbitrary npm packages.
...
Write a useful description for JSX snippets and clear content for MDX snippets. MDX body content is indexed with importing pages, while a JSX snippet's `description` is used as its searchable text.
