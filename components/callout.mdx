...
<ParamField path="icon" param-type="string" required="false" deprecated="false">
  ...
  Override the default icon with a Lucide icon name (e.g. `lightbulb`, `shield-alert`, `rocket`). When provided, this replaces the icon associated with the `kind` prop. See the [Lucide icon gallery](https://lucide.dev/icons) for available names.

</ParamField>

<ParamField path="color" param-type="string" required="false" deprecated="false">

  Override the default color with a hex color code (e.g. `#7c3aed`). When provided, this sets the accent color for the callout border, icon, and background tint, overriding the color associated with the `kind` prop.

</ParamField>

### Custom icon and color

You can customize any callout with a specific Lucide icon and accent color, regardless of the `kind`:

```jsx
<Callout kind="info" icon="lightbulb" color="#7c3aed">

  This callout uses a custom lightbulb icon and purple accent.

</Callout>
```

```jsx
<Callout kind="alert" icon="shield-alert" color="#dc2626">

  This callout uses a shield-alert icon and red accent for security warnings.
