...
---
<Update label="2026-06-30" description="SVG block rendering fixes in the editor" tags={["fix"]}>
  ## Bug Fixes

  - **SVG Block rendering** — SVG blocks now render correctly when you upload an SVG file or select one from your media gallery. Previously, uploaded SVGs could fail to display due to URL handling issues that prevented the editor.

  - **Malformed SVG error handling** — Invalid or malformed SVG files now show a clear error message instead of rendering blank or broken content.

  See [Web Editor](/write-and-publish/web-editor) for more on editor components.
</Update>
