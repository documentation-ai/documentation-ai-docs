...
A successful selection makes the current project available for subsequent tool calls. Your selection persists across sessions, so you only need to select a project once. If your session is interrupted, the selection is automatically restored when you reconnect.
