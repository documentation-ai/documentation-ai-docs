...
If your documentation is deployed at a subpath, such as `example.com/docs`, redirects are normalized against that subpath. You can include or omit the deployment subpath in internal redirect sources and destinations; Documentation.AI resolves both forms to the same final URL.

For a subpath deployment at `/docs`, choose either form. These two alternatives normalize to the same redirect, so you do not need to configure both.
...
Both alternatives normalize to:

- Request to `/docs/old-page` → Redirects to `/docs/new-page`

External destinations are not prefixed with the deployment subpath:
...
```json
{
  "redirects": [
    {
      "source": "/old-page",
      "destination": "https://status.documentation.ai/incidents"
    }
  ]
}
```
...
- Request to `/docs/old-page` → Redirects to `https://status.documentation.ai/incidents`

Subpath matching uses strict path boundaries. A deployment subpath of `/docs` matches `/docs` and `/docs/old-page`, but it does not match `/docs-extra` or `/docs-extra/old-page`.
...
<ExpandableGroup>
  ...
  <Expandable title="Test redirects before deploying" default-open="false">
    ...
    4. For subpath deployments, test sources and destinations with and without the deployment subpath

    5. Confirm dynamic parameters are captured correctly
  </Expandable>
...
<ExpandableGroup>
  <Expandable title="Redirect not working" default-open="false">
    ...
    - For subpath deployments, make sure the request is inside the exact deployment subpath. For example, `/docs-extra` does not match a `/docs` deployment.
  ...
  <Expandable title="Wrong destination URL" default-open="false">
    ...
    - For subpath deployments, internal destinations may include or omit the deployment subpath. External destinations are not prefixed.
  </Expandable>
