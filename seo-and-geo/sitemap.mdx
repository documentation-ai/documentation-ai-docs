...
Each sitemap entry also includes an accurate `lastModified` date for that page. Documentation.AI uses the page's actual last update time, not the deployment date, so search engines receive better freshness signals and can crawl your docs more efficiently.
