...
You always add Views _inside_ the selected Dimension branch.
...
- Use **Add Child layer** (bottom <kbd>+</kbd>) when you only want to add structure _inside_ the selected item (for example, a new tab inside a version).
...
## Configuring Views in documentation.json

For file-based management, define tabs and dropdowns directly in `documentation.json`. This is useful when you are making changes via Code editor or for doing bulk changes 

### Tabs at the root level (no dimensions)

Define a `tabs` array at the root of your navigation. Each tab contains groups or pages:
...
### Dropdowns under a tab

Use the `dropdowns` key inside a tab to create sidebar categories:
...
Dropdowns can contain other dropdowns for deeper hierarchy:
            ...
            "dropdowns": [
              {
                ...
                "pages": [
                  { "title": "API Keys", "path": "api/auth/keys" }
                ]
              }
            ]
          }
...
### Views inside dimensions

When using dimensions, define views inside each dimension value. For example, tabs inside a version:
            ...
            "groups": [
              {
                "group": "Getting Started",
                "pages": [
                  { "title": "Introduction", "path": "v2/introduction" }
                ]
              }
            ]
          }
