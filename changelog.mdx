...
---
<Update label="2026-06-29" description="HTML Block and SVG Block editor components, unknown MDX component preservation, and onboarding logo fix" tags={["feature","improvement","fix"]}>

  ## HTML Block

  Embed custom HTML directly in your documentation pages with a live preview. Use the slash command and search for "HTML Block" to insert it. Toggle between editing your HTML markup and seeing the rendered result inline, with full style isolation so your custom styles stay scoped to the block. See [Web Editor](/write-and-publish/web-editor).

  ## SVG Block

  Add vector SVG graphics to your pages with a built-in upload dialog. Insert an SVG by uploading a file, pasting raw markup, or selecting from your existing image library. SVGs render responsively inline with no extra configuration. See [Web Editor](/write-and-publish/web-editor).

  ## Unknown MDX Components Preserved

  When importing MDX that contains components the editor doesn't recognize, those components are now preserved as editable HTML Blocks instead of being silently dropped. Your custom widgets and third-party embeds stay intact when you open existing content in the editor.

  ## Bug Fixes

  - **Onboarding Logo Upload** — Logos uploaded during the onboarding brand setup step now reliably resolve to the correct URL and appear as expected.
