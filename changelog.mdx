...
---
<Update label="2026-09-08" description="Media Library, AI assistant reliability, reader experience fixes, and access control enforcement" tags={["feature","improvement","fix"]}>
  ## Media Library
  ...
  - Replaced images refresh across the CDN immediately instead of serving stale cached versions
  ...
  - Browse large libraries with configurable pagination (30, 60, or 120 items per page) and auto-loading in the editor sidebar
  ...
  Per-organization storage quotas are enforced: uploads and new publishes pause when you exceed your quota. Already-published sites continue serving.
  ...
  - **AI streaming keepalive**: Long-running AI responses now send periodic keepalive signals to prevent timeouts from proxies and load balancers.
  - **Per-file limits removed from storage settings**: The per-file limit section has been removed from the media storage settings panel. File size limits still apply; see the [limits table](/write-and-publish/media-library#limits-and-supported-file-types) on the Media Library page.
  - **Cached pages for authenticated readers**: Signed-in readers on sites without role-based page restrictions now receive cached pages, improving load performance while keeping responses private and non-indexable.

  ## Fixes

  - **Page renames and moves across directories**: Renaming or moving a page across directories now correctly updates all paths on the server, in the editor, and in `documentation.json`. Missing destination folders are created automatically, and existing destinations are rejected instead of being silently overwritten.
  - **Role-based access enforcement**: Role-based page restrictions are now enforced only when the site's role-based access setting is enabled. Password-authenticated sites no longer apply stale role mappings left over from previous JWT or OAuth settings, and anonymous readers of protected sites no longer see gated page names and paths in the navigation.
  - **PDF export in Atlas theme**: Long pages now paginate fully instead of cutting off at the visible viewport. The sticky header and chat widget are excluded from PDF output.
  ...
  - **OAuth redirect URI behind reverse proxy**: Fixed OAuth login failures for documentation sites behind a reverse proxy. OAuth callback URLs now use the customer-facing domain from forwarded host headers instead of the internal service URL.
  - **Repository link handling**: Repository and commit links in the dashboard now correctly handle URLs ending in `.git`, trailing slashes, and differences between GitHub and GitLab route patterns.
</Update>
