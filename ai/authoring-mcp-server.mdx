...
All tools declare read-only and destructive annotations. Your MCP client can use these hints to skip confirmation prompts for safe operations like `get_page` or `search_documentation`, while still warning before destructive operations like `update_documentation`.
