...
---
<Update label="2026-07-03" description="Branch-based editor URLs, incremental MDX validation, faster search, and reliability fixes" tags={["improvement","fix"]}>

  ## Branch-Based Editor URLs

  The editor now uses branch-aware URLs (`/editor/[branch]`), so each branch has its own address in the dashboard. This means your browser's back and forward buttons work correctly when switching between branches, and you can bookmark or share a direct link to a specific branch's editor view.

  When you switch branches from the branch selector, the URL updates automatically. The "View Changes" action from workflow run history now navigates directly to the editor with the diff dialog pre-opened, so you can review what changed without extra clicks.

  ## Incremental MDX Validation

  Deployments now validate only the MDX files that changed in the triggering commit instead of re-validating the entire documentation site. This significantly reduces deployment time for sites with many pages, especially when only a few files were updated. Full validation still runs for cache-bypass deployments.

  ## Improvements

  - **Search Debounce**: Reduced the search input delay from 300ms to 200ms for snappier search feedback as you type.

  - **Expandable Rendering**: Switched from CSS visibility toggling to conditional rendering for expandable sections, fixing layout issues with nested expandables inside ResponseField and other container components.

  - **Attachment Upload Reliability**: Fixed race conditions in the AI agent chat where in-flight file uploads could be attributed to a deleted thread. Uploads are now properly cancelled and cleaned up when a thread is removed, and pending attachments are cleared on reset to prevent stale files from being included in future messages.

  - **API Token Auto-Retry**: API requests that receive a 401 due to an expired auth token now automatically retry once with a refreshed token, preventing false authentication failures in production.

  - **Branch Preview Subdomains**: Preview subdomains are now guaranteed to fit within DNS label limits, preventing invalid URLs for branches with long names.
...
<Update label="2026-06-30" description="PDF export, SVG and HTML Block components, SEO meta overrides, API playground server management, heading copy-link, and platform improvements" tags={["feature","improvement","fix"]}>
  ...
  - **Heading Copy Link**: Clicking any heading on a published page now copies its anchor link to the clipboard and updates the browser URL hash. A brief check-icon animation confirms the copy, making it easy to share deep links to specific sections.
  ...
  - **White-Label Branding**: Remove Documentation.AI branding from your published documentation site for a fully white-labeled experience. Available as a toggle in billing settings for qualifying plans.

  - **Credit Pack Purchases**: Buy additional AI credits directly from billing settings when you need more before the next cycle. Low-credit alerts in the sidebar and dashboard notify you when you're running low.

  - **Callout Custom Icons & Colors**: The Callout component now supports custom `icon` and `color` props, letting you tailor callout appearance beyond the built-in kinds. See [Callout](/components/callout).
...
<Update label="2026-05-11" description="AI Workflows, branches and previews, AI Assistant customization, and platform improvements" tags={["feature","improvement","fix"]}>
  ...
  - **Container Content Pages**: Navigation containers such as groups, products, and tabs can now have their own content pages instead of only acting as wrappers. See [Pages](/organize/pages) and [Views](/organize/views).
