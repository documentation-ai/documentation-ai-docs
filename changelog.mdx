...
---
<Update label="2026-07-15" description="Enterprise plan, Callout custom icons and colors, collaboration merge tracking, and MCP OAuth reliability" tags={["feature","improvement","fix"]}>

  ## Enterprise Plan

  A new Enterprise plan is available for larger teams that need higher limits and dedicated support. The plan includes 50,000 AI credits per month, 5 documentation sites, and 20 team seats. Enterprise subscriptions are provisioned by the sales team rather than self-serve checkout. [Contact us](mailto:hello@documentation.ai) to get started.

  ## Improvements

  - **Callout custom icons and colors**: The Callout component now accepts `icon` and `color` props. Pass any Lucide icon name in kebab-case to `icon` and any valid CSS color value to `color` to override the default appearance. See [Callout](/components/callout).

  - **Collaboration merge tracking**: When a collaborator initiates a merge, other editors on the same branch see a toast notification and the editor enters a read-only state until the merge completes. After the merge, each collaborator's workspace reloads with the merged content. This prevents conflicting edits during merge operations and keeps everyone in sync. See [Real-Time Collaboration](/write-and-publish/collaboration).

  ## Fixes

  - **MCP OAuth session persistence**: Project selection for Authoring MCP Server OAuth sessions now persists across server restarts, connection drops, and session re-initializations. Previously, OAuth users had to re-select their project after a session loss, which caused all subsequent tool calls to fail. See [Authoring MCP Server](/ai/authoring-mcp-server).
