...
---
<Update label="2026-06-23" description="SEO frontmatter fields, API playground upgrades, navigation icons, sitemap metadata, and search and UI fixes" tags={["feature","improvement","fix"]}>
  ## Features

  - **SEO frontmatter fields** — Add `metaTitle` and `metaDescription` in page frontmatter to control browser titles and search snippets without changing the visible page title.

  - **API playground server management** — Manage available servers directly in the playground so readers can switch environments more easily while testing endpoints.

  - **API playground input examples** — Parameter and request inputs now use clearer example values, making it faster to run requests with realistic defaults.

  - **Navigation icons** — Add icons to groups, tabs, dropdowns, and individual pages to make large documentation sets easier to scan.

  - **Callout customization** — Set a custom icon and color on Callout components when you need a notice to match a specific meaning or visual style.

  ## Improvements

  - **Sitemap metadata** — Sitemap entries now include per-page `lastModified` timestamps so search engines can better understand when individual pages changed.

  ## Bug fixes

  - **Search ranking** — Improved ranking for pages with short titles and for queries that match the page title directly.

  - **Expandable titles** — Fixed long Expandable titles getting truncated incorrectly in the UI.
</Update>
