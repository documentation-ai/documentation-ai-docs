...
---
<Update label="2026-07-21" description="Enterprise plan, collaborative editing for all file types, MCP project persistence, and collaboration improvements" tags={["feature","improvement","fix"]}>

  ## Enterprise Plan

  A new Enterprise billing tier is now available for teams with larger documentation operations. The plan includes 20 editor seats, 50,000 AI credits per month, 5 documentation sites, and API key creation access. Enterprise plans are sales-led and can be customized beyond these base limits.

  To upgrade to Enterprise, [contact our team](mailto:hello@documentation.ai).

  ## Collaborative Editing for All File Types

  Real-time collaboration now extends beyond MDX pages to all file types in your documentation project, including JSON files, OpenAPI specs, custom CSS, and scripts. Changes sync live across all connected collaborators, so teams can coordinate edits to any file without manual refreshes.

  File operations such as create, delete, rename, and move also sync in real time during collaboration sessions.

  ## Improvements

  - **MCP project selection persistence**: Project selection in the Authoring MCP Server now persists across sessions. Users only need to select their project once, and the selection is automatically restored on reconnection.

  - **Real-time merge status**: When a collaborator initiates a branch merge, all connected editors see the merge in progress with the user's name. A completion notification confirms the merge result.

  - **Tree operation feedback**: File operations in the editor now show toast notifications while in progress, and the publish button is disabled during active tree operations to prevent conflicts.

  - **Expandable component polish**: The Expandable component header is now more compact with consistent padding, and content panels render more reliably.

  ## Bug Fixes

  - **Folder rename/move safety**: Fixed an issue where renaming or moving a folder that contained files never opened in the editor could result in content loss on publish.

  - **Agent-created pages**: Fixed an issue where pages created by the AI agent through the REST API could display an empty editor when opened, despite having correct content on the published site.
