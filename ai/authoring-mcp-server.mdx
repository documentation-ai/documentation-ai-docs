...
Connect your MCP client to the server URL, then authenticate with either OAuth 2.1 or a project API key.

```text
https://api.documentationai.app/mcp
```

Choose the authentication mode that matches your workflow:

- **OAuth 2.1** — Best for interactive MCP clients where a user signs in with a Documentation.AI account. OAuth sessions can access every project the signed-in user is allowed to use.
- **API key** — Best for automation, CI workflows, or dedicated agents. API-key sessions are scoped directly to the documentation project that owns the key.
...
Open your MCP-compatible client and add the Authoring MCP Server using the URL in the next section.
  ...
  <Step title="Authenticate the session" icon="lock">

For OAuth, your client discovers the OAuth endpoints and opens a browser window for sign-in. For API-key authentication, configure your MCP client to send the project API key as a bearer token.

  </Step>
  <Step title="Confirm access" icon="check-circle">

After OAuth approval or API-key validation succeeds, the client completes the connection.
...
OAuth does not require manual token management. API-key authentication uses the same project-scoped keys that power the REST API. Because an API-key session is already bound to one project, it does not need `list_projects` or `select_project` before running documentation tools.
...
Project selection depends on how the session authenticated.

- **OAuth sessions** must select the documentation project to work on before using documentation tools.
- **API-key sessions** are already scoped to the project that owns the key, so project selection is not required.

<Steps>
  <Step title="List available projects" icon="folder">

For OAuth sessions, call `list_projects` to see every organization and documentation project your account can access.
...
In an OAuth session, you can switch projects at any time by calling `select_project` again. In an API-key session, create or provide a different project API key to work with another project.
...
Merges one branch into another. Merging into the deployment branch triggers a live deployment. Merging into a non-deployment branch triggers a preview deployment for the target branch when previews are available.
