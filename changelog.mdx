...
---
<Update label="2026-08-18" description="Broken Link Audit, workflow template categories, seat limit increases, and llms.txt auth support" tags={["feature","improvement","fix"]}>
  ## Broken Link Audit

  The Broken Link Audit workflow is now fully implemented. It scans your published live site for broken internal links, anchors, redirects, and external URLs, then produces a report with an AI-written summary. The audit runs as a durable background job and handles large sites with thousands of pages.

  - Scans internal links, anchor links, redirect chains, external URLs, navbar buttons, and redirect destinations
  - External links are checked with HEAD requests and a fallback scraper for bot-walled pages
  - Results are grouped by issue type and include a natural-language summary
  - Use **Chat with Agent** on a finished run to open the findings in the editor chat and propose fixes

  See [Workflows](/ai/workflows) for setup instructions.

  ## Workflow Template Categories

  Built-in workflow templates are now organized into two categories in the template picker:

  - **Self-updating**: Templates that generate or update documentation content (Documentation Update from Code, API Sync from Code, Changelog Generation, Self-Heal by User Feedback)
  - **Auditing**: Templates that check and report on documentation health (Broken Link Audit, Grammar and Typo Check, Style Guide Enforcement, Navigation Audit)

  The picker UI has been redesigned with cleaner card layouts and more descriptive icons.

  ## Improvements

  - **Chat with Agent on finished runs**: Completed workflow runs that produced findings now show a **Chat with Agent** button alongside the existing report and commit link. Clicking it opens the editor chat pre-loaded with the run's findings so you can ask follow-up questions or request fixes.
  - **Seat limit increases**: All paid plans now include more editor seats. Starter increased from 1 to 5, Standard from 3 to 10, and Professional from 10 to 20. See [Usage Limits](/account-and-billing/usage-limits).
  ...
  - **External links in navigation tabs and dropdowns**: External links configured in tabs and dropdowns are now preserved on auth-filtered sites and open in a new browser tab.
  ...
  - **Deferred syntax highlighting during AI streaming**: Code blocks in AI agent responses render as plain text while the response is streaming, then switch to full syntax highlighting once the message completes. This eliminates re-rendering flicker during generation.
