...
---
<Update label="2026-07-02" description="Branch-aware editor URLs, workflow View Changes button, and UI cleanup" tags={["feature","improvement"]}>

  ## Branch-Aware Editor URLs

  The editor URL now includes the active branch name (e.g. `/editor/main`, `/editor/feature-branch`). You can bookmark, share, or deep-link to a specific branch, and browser back and forward navigation moves between branches you've visited.

  - Opening a direct link to a specific branch loads that branch immediately instead of defaulting to your last-used branch.
  - The sidebar **Editor** link always points to your current branch, and the nav item stays highlighted on any branch sub-route.

  See [Branches and Previews](/write-and-publish/branches-and-previews).

  ## Workflow "View Changes" Button

  From the Workflows list, clicking **View Changes** on a completed run now navigates directly to the editor on the relevant branch and opens the diff dialog automatically — no extra clicks needed. This replaces the previous "Open in Chat," download, and commit link actions with a single, clearer path to review what a workflow changed.

  See [Workflows](/ai/workflows).

  ## Improvements

  - **Disabled workflows hidden** — Workflow templates that aren't available yet no longer appear in the creation flow, template picker, or Workflows list. Previously they showed as greyed-out options.
