...
**Publish** is available on the Live branch. Use it when you want changes already on the deployment branch to go live on your site.

### Use Save & Merge to send a branch to Live

**Save & Merge** saves the current branch and merges it into the Live branch. Because the Live branch is the deployment branch, a successful merge starts a live deployment automatically.

<Callout kind="info" collapsed="false">

  Merge deployment behavior depends on the target branch. Merging into the Live branch starts a live deployment, while merging into a non-live branch updates or creates that branch's preview deployment when preview sites are available.
...
<Steps>
  ...
  <Step title="Wait for the live deployment" icon="check-circle" title-type="p">
    After the merge succeeds, the editor switches you back to the Live branch automatically and starts deploying the merged content to the live site.

    Wait for the deployment to finish before treating the changes as live for visitors.
  </Step>
</Steps>
...
When a pull request is merged into the Live branch through Git, Documentation.AI treats that merge the same way as a dashboard merge into Live: the updated deployment branch is deployed to the live site.
...
Branches give you a safer way to work than editing directly on the Live branch. You can create a branch from the editor top bar, choose whether to carry local unsaved changes, review a preview site when available, and then save for later, merge to Live for deployment, publish directly from Live, or create a pull request.
