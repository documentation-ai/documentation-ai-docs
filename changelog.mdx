...
<Update label="2026-06-04" description="Board component, AI Assistant refinements, navigation fixes, and authoring updates" tags={["feature","improvement","fix"]}>
  ## Board component

  The new [Board component](/components/board) lets you publish read-only kanban-style boards in MDX. Use boards for roadmaps, editorial workflows, feature inventories, and other status-based layouts where readers need to scan columns and open cards for details.

  ## Content and component updates

  - **GIF handling in images** — Image documentation now clarifies how GIFs behave when uploaded and rendered in docs. See [Images](/components/images).

  - **Metadata descriptions for SEO and social previews** — Page configuration now explains how encoded metadata descriptions are decoded before they appear in search, social previews, and page metadata. See [Page Configuration](/customize/page-configuration).

  - **Navbar button label casing** — Header and site configuration docs now clarify how primary navbar button labels render. See [Header and Navbar](/customize/header-and-navbar) and [Site Configuration](/customize/site-configuration).

  ## AI and authoring improvements

  - **AI Assistant behavior** — AI Assistant docs now cover updated feedback behavior, more tolerant search matching, fallback guidance, and CAPTCHA protection. See [AI Assistant](/ai/ai-assistant).

  - **AI analytics context** — Analytics documentation now reflects the latest AI Assistant visibility and feedback signals. See [Analytics overview](/analytics/overview).

  - **Authoring MCP authentication** — Authoring MCP docs now explain how to connect MCP-compatible clients using API-key authentication. See [Authoring MCP Server](/ai/authoring-mcp-server) and [MCP Server overview](/ai/mcp-server).

  - **AI Documentation Agent attachments** — AI agent docs now include clearer attachment limit guidance so teams can add context files more predictably. See [AI Documentation Agent](/ai/ai-documentation-agent).

  ## Routing and navigation fixes

  - **Redirect and rewrite behavior** — Redirect documentation now describes the current matching behavior for custom subpaths and rewrite rules. See [Redirects](/customize/redirects-and-rewrites).

  - **Custom subpath matching** — Custom subpath setup docs now clarify how Documentation.AI matches and serves documentation under a subpath. See [Custom Subpath Overview](/customize/custom-subpath/overview).
</Update>
