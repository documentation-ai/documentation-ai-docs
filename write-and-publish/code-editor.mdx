...
<Steps>
  ...
  Preview URLs are based on the branch name and your documentation subdomain. Documentation.AI normalizes that combination into a URL-safe hostname, so the preview URL may not exactly match the original branch name when it contains punctuation, spaces, or special characters.
...
- **Page builds but content is broken**  
  ...
  - To write literal braces, wrap the text in inline code, such as `{customerId}`, or escape braces as `&#123;` and `&#125;` when they should render as normal text.  
