---
title: Usage Limits
description: Understand your plan limits and monitor your organization's usage.
---
## Plan Usage Limits Overview

### Starter Plan (Free)

- **Editor Seats**: 5 users can edit documentation

- **AI Credits**: 0 recurring (includes one-time 10,000-credit trial, expires after 14 days)

- **Reset Period**: Monthly on your billing date

### Standard Plan ($59/month, $49/year)

- **Editor Seats**: 10 users can edit documentation

- **AI Credits**: 10,000 credits per month

- **Reset Period**: Monthly on your billing date

### Professional Plan ($149/month, $119/year)

- **Editor Seats**: 20 users can edit documentation

- **AI Credits**: 30,000 credits per month

- **Reset Period**: Monthly on your billing date

### Enterprise Plan (Custom)

- **Editor Seats**: Unlimited or custom limit

- **AI Credits**: 50,000 credits per month

- **Reset Period**: Based on your agreement

## Understanding Editor Seats

### What Counts as an Editor Seat

- Users with **Admin or Editor role** in your organization

- Users who can create, edit, or publish documentation

- **Viewer role** users do NOT count toward editor seat limits

### When You Reach Editor Limits

- Cannot invite additional Admin users

- Existing Admin users retain full access

- Must upgrade plan or remove existing editors to add new ones

### Managing Editor Seats

1. Go to **Settings** → **Organization Settings** → **Members**

2. View current Admin users (these count as editor seats)

3. Change user roles from Admin to Viewer to free up seats

4. Remove users who no longer need editing access

## AI Credit Limits

### What Consumes Credits

Each AI Assistant question costs exactly 1 credit. The AI Documentation Agent uses credits based on task complexity. See [AI Credits](/ai/ai-credits) for detailed credit costs per task type.

### When You Run Out of Credits

AI features are disabled until your next credit cycle or until you purchase additional credits. Purchase credit packs or enable overage billing from Settings → Organization Settings → Billing. See [AI Credits](/ai/ai-credits) for details.

### Managing AI Usage

Monitor credit usage in Settings → AI Usage. See [AI Credits](/ai/ai-credits) for tips on using credits efficiently.

## Media Storage

Images, video, audio, and files uploaded to the [media library](/write-and-publish/media-library) count toward one storage quota shared by every documentation in your organization. Each plan also caps the size of a single file by type.

| Plan         | Total storage | Images | Video and audio | Documents and files |
| ------------ | ------------- | ------ | --------------- | ------------------- |
| Starter      | 1 GB          | 10 MB  | 20 MB           | 10 MB               |
| Standard     | 5 GB          | 10 MB  | 30 MB           | 20 MB               |
| Professional | 20 GB         | 10 MB  | 50 MB           | 30 MB               |
| Enterprise   | 100 GB        | 10 MB  | 50 MB           | 30 MB               |

AppSumo Tier 1 and 2 use the Standard limits; Tier 3 and 4 use the Professional limits.

### Supported File Types

| Kind      | Formats                                                    | Size limit column     |
| --------- | ---------------------------------------------------------- | --------------------- |
| Images    | JPEG, PNG, GIF, WebP, SVG, ICO, HEIC/HEIF, AVIF, BMP, TIFF | Images                |
| Video     | MP4, M4V, WebM, MOV                                        | Video and audio       |
| Audio     | MP3, WAV, OGG, M4A, AAC                                    | Video and audio       |
| Documents | PDF, DOCX, DOC, XLSX, XLS, PPTX, PPT, ODT, ODS, ODP        | Documents and files   |
| Data      | CSV, TSV, JSON, YAML, XML, TXT, LOG, MD, MDX               | Documents and files   |
| Archives  | ZIP, GZ, TGZ, TAR                                          | Documents and files   |
| Fonts     | WOFF, WOFF2, TTF, OTF                                      | Documents and files   |

Never accepted, on any plan: programs and installers (`.exe`, `.msi`, `.dmg`, `.apk`, `.jar`), scripts (`.js`, `.sh`, `.ps1`, `.bat`), macro-enabled Office files (`.docm`, `.xlsm`, `.pptm`), and web pages (`.html`, `.htm`). Uploads are verified on the server: the file's contents must match its extension, and SVG files containing scripts or embedded documents are rejected.

### What Counts Toward Storage

- Every file in a documentation's media library, including poster frames generated for uploaded videos

- Files uploaded before the media library existed do not count until they are adopted into the library

- AI assistant attachments, PDF exports, and OpenAPI specs are not counted

### When You Reach Storage Limits

- **80% used**: A warning appears in the media library and in Media Storage settings

- **100% used**: New uploads are refused and publishing is blocked until usage is back under the limit

- Already-published sites keep serving, and deleting files always works

### Managing Media Storage

1. Go to **Settings** → **Organization Settings** → **Media Storage**

2. Review used space, per-file limits, and (as an admin) usage by documentation

3. Delete unused files from the media library, or upgrade to a plan with more storage

## Monitoring Your Usage

### Check Current Usage

1. Go to **Settings** → **Organization Settings** → **Billing**

2. View usage dashboard showing:

   - Current editor seats used

   - AI credits used this month

   - Days remaining until reset

### Usage Alerts

- **75% warning**: Email notification when approaching limits

- **90% warning**: Dashboard notification for heavy usage

- **100% reached**: Service limitations take effect

## What Happens at Reset

### Monthly Reset Schedule

- **Usage resets** on your billing date each month

- **AI credits are replenished based on your plan allocation**

- **Editor seats** are ongoing (not reset monthly)

### Billing Date Examples

- If you subscribed on the 15th, usage resets on the 15th each month

- Free plan users also get monthly resets

- Enterprise customers may have custom reset schedules

## Optimizing Your Usage

### Editor Seat Optimization

- Use **Viewer roles** for team members who only need read access

- Regularly review who needs editing permissions

- Remove inactive users to free up seats

### AI Assistant Optimization

- Use AI assistant for complex questions

- Train team members on effective AI queries

- Consider purchasing credit packs, enabling overage, or upgrading your plan. See [AI Credits](/ai/ai-credits) for details.

## Plan Recommendations

### When to Consider Upgrading

**Upgrade to Standard if:**

- You need 6-10 editor seats

- Need AI credits beyond the Starter trial

- Want advanced grammar and spelling features

**Upgrade to Professional if:**

- You need 11-20 editor seats

- Need more than 10,000 AI credits per month

- Need preview deployments or private documentation

**Consider Enterprise if:**

- You need more than 20 editor seats

- Need more than 30,000 AI credits per month

- Need advanced security or compliance features

## Troubleshooting

**Can't add new team members?** Check if you've reached your editor seat limit and consider upgrading or changing existing user roles.

**AI assistant not working?** You may have run out of credits. Check your balance in Settings → AI Usage.

**Usage seems incorrect?** Contact support to review your account usage metrics.

**Need temporary limit increase?** Contact support for special circumstances or consider upgrading.

## Related Topics

- [Upgrade / Downgrade Plans](upgrade-downgrade) - Change your limits

- [Team Management](team-management) - Manage editor seats

- [Billing & Subscriptions](billing-and-subscriptions) - View detailed usage metrics