...
<Update label="2026-06-01" description="Board components, fuzzy search, and recent fixes" tags={["feature","improvement","fix"]}>
  ## New features

  - **Board components** — Build kanban-style documentation layouts for roadmaps, workflows, and status boards using the component system. See [Components](/components/components).

  ## Updates

  - **Fuzzy search** — Search now finds more helpful results when readers make typos or use slightly different wording.

  ## Bug fixes

  - **Subpath redirects** — Fixed redirects for docs served from a custom subpath so redirect rules resolve consistently. See [Redirects](/customize/redirects-and-rewrites).

  - **Italic text inside links** — Fixed rendering for links that include italic text so linked labels keep the expected styling.
</Update>
