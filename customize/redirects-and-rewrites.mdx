...
If your documentation is deployed at a subpath, such as `example.com/docs`, Documentation.AI normalizes redirects so they work whether your configured `source` includes the subpath or not.

Both of these configurations match requests under `/docs` when your deployment subpath is `/docs`:
...
```json
{
  "redirects": [
    ...
    {
      "source": "/docs/legacy-page",
      "destination": "/docs/current-page"
    }
  ]
}
```
...
- `/docs/legacy-page` → `/docs/current-page`

Documentation.AI avoids double-prepending the subpath. If the `source` or `destination` already starts with `/docs`, the platform uses that path as-is instead of turning it into `/docs/docs/...`.

### Path boundary matching

Subpath matching uses strict path boundaries. A deployment subpath only matches the exact subpath segment, not every path with the same prefix.

For a subpath deployment at `/docs`:

- `/docs/old-page` is treated as documentation traffic
- `/docs-v2/old-page` is not treated as documentation traffic
- `/documentation/old-page` is not treated as documentation traffic

This prevents similar prefixes from being confused with your configured subpath.

### External destinations

External redirect destinations remain external. Documentation.AI does not rewrite absolute URLs under your deployment subpath.
...
```json
{
  "redirects": [
    {
      "source": "/old-api",
      "destination": "https://developer.example.com/api"
    }
  ]
}
```

For a subpath deployment at `/docs`, a request to `/docs/old-api` redirects to `https://developer.example.com/api`, not `/docs/https://developer.example.com/api`.
