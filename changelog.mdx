...
<Update label="2026-09-02" description="Reusable snippets, search analytics, Intercom metadata, and bug fixes" tags={["feature","improvement","fix"]}>
  ...
  Create reusable `.mdx` and `.jsx` snippets under the `snippets/` directory and insert them into multiple pages. Snippets stay in sync: edit a snippet once, and every page that uses it updates automatically on the next deployment.
  ...
  - Support for both `.mdx` prose snippets (with variable substitution like `{name}`) and `.jsx` component snippets (compiled and rendered safely)
  - Nesting support: `.mdx` snippets can import other snippets up to a depth limit
  ...
  - Searchable event table with filters and pagination, including individual search queries, results shown, clicks, and AI escalations
  ...
  - **Intercom user metadata**: The Intercom widget now sends organization ID, organization name, plan type, documentation ID, and documentation name as custom attributes on each conversation, giving your support team instant context about the user's account. See [Intercom](/integrations/intercom).
  ...
  - **OAuth redirect URI behind reverse proxies**: OAuth 2.0 initiation and callback now derive the redirect origin from `x-forwarded-host` and `x-forwarded-proto` headers, so providers such as Azure AD receive your public URL instead of the internal service URL.
</Update>
