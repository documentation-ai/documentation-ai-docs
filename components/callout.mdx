...
- `custom`: A fully custom callout with no built-in kind styling.
...
In MDX, use the `Callout` component with a `kind` attribute to control the visual style. The `kind` value must be one of `info`, `tip`, `success`, `alert`, `danger`, or `custom`.
...
<ParamField path="kind" param-type="string" required="false" deprecated="false">
  Visual style for the callout. One of: `info` (default), `tip`, `success`, `alert`, `danger`, or `custom`. Use `custom` when you want to remove the built-in kind styling and define your own icon or color.
</ParamField>

<ParamField path="icon" param-type="string" required="false" deprecated="false">
  Replaces the default kind-based icon with any Lucide icon name in kebab-case, such as `rocket` or `shield-alert`. If the icon name is invalid, the callout falls back to the default icon for the selected kind.
</ParamField>

<ParamField path="color" param-type="string" required="false" deprecated="false">
  Applies a custom CSS color value to the icon and a subtle tinted background. You can use hex, RGB, HSL, or named colors.
</ParamField>

### Custom styling

Use `icon` and `color` when you want more control over how a callout looks without changing the content structure.

#### Override the icon

```jsx
<Callout kind="info" icon="rocket">
  Ship your docs update after you review the preview branch.
</Callout>
```
...
```jsx
<Callout kind="custom" color="#7c3aed">
  Match callouts to your brand or highlight a specific workflow.
</Callout>
```

#### Combine a custom icon and color

```jsx
<Callout kind="custom" icon="shield-alert" color="rgb(220, 38, 38)">
  Review access settings before you publish internal documentation.
</Callout>
```
