...
Media files are not committed to the repository. Upload them to the documentation's [media library](/write-and-publish/media-library#code-editor-git) from the dashboard, copy the CDN URL, and reference it in MDX:
...
Replacing a file in the library keeps its URL, so pages update without a commit. See [Images](/components/images), [Videos and iframes](/components/videos-and-iframes), and [Files](/components/files) for props.

To upload files from a script or a CI/CD pipeline instead of the dashboard, use the [REST API](/api-reference/overview#media).
...
If the build still fails, [contact us](/help/contact-us).
