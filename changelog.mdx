...
---
<Update label="2026-06-08" description="SVG block links now work correctly in the editor" tags={["fix"]}>

  ## Bug Fixes

  - **SVG Block Links** — Hyperlinks embedded inside SVG images now work correctly in the editor. Links display a pointer cursor on hover and open the target URL in a new tab when clicked. This supports both modern and legacy SVG link formats, so SVGs exported from tools like Graphviz and Inkscape work as expected. Previously, links in SVG blocks were not clickable and gave no visual indication they were interactive. See [Images](/components/images).
