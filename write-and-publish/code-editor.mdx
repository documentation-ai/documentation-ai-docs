...
Use quoted string props for stable, readable MDX:
...
Documentation.AI also normalizes common authoring mistakes so pages keep rendering while you fix them:

- Simple bare identifier props, such as `kind=info`, are treated as string values.
- Brace-wrapped kebab-case tokens, such as `{release-notes}`, render as literal text instead of breaking the page.

<Callout kind="tip">

Treat this tolerance as a safety net, not a style guide. Prefer explicit quoted props in committed docs because they are easier to review, search, and reuse across MDX tooling.
