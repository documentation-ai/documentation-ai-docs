...
<Update label="2026-08-18" description="Broken Link Audit, seat limit increases, and llms.txt auth support" tags={["feature","improvement","fix"]}>
  ...
  All paid plans now include more editor seats: Starter 1→5, Standard 3→10, Professional 10→20.&#x20;
...
<Update label="2026-08-11" description="Self-Heal by User Feedback workflow, analytics dashboard restructure, branch recovery, per-user content, and critical fixes" tags={["feature","improvement","fix"]}>
  ...
  The analytics dashboard was restructured with a new tab layout. The **Overview** tab is now the default, with stat cards showing period-over-period trends for human views, AI agent views, user feedback, and AI assistant usage. The **Traffic** tab breaks down page views by audience with sub-tabs for People, AI Agents, and Bots and Crawlers. Each tab loads its data independently for faster navigation.

  Bot and AI agent visits are now tracked separately, so you can see which pages are read by AI tools like Claude, ChatGPT, Cursor, and Perplexity, plus search engine crawlers and other automated agents.
  ...
  - **GitLab branch-not-found errors**: GitLab-specific error messages are now handled correctly instead of showing raw error text.
</Update>
