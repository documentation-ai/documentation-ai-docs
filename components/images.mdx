...
Use images to illustrate concepts, show UI, and make your documentation easier to scan. Documentation.AI automatically optimizes supported still images uploaded through the Web Editor and serves uploaded assets from a CDN, so you get:

- Fast, cached delivery for uploaded images, including GIFs.

- Responsive sizing with preserved aspect ratio.

- Built-in support for accessibility via required `alt` text.

Animated GIFs are supported and cached, but they are served without optimization transforms. This preserves animation and avoids slow first loads caused by transforming large animated files.
...
- **CDN:** Images uploaded via the Web Editor are automatically cached. Supported still image formats are optimized by the CDN; animated GIFs are delivered without optimization transforms so the animation remains intact and first loads stay responsive.
