...
Hover near the left edge of a row to reveal the row handle button (⋮). Click it to open the row menu:

- **Insert row above** – Add a new row above the current one.
- **Insert row below** – Add a new row below the current one.
...
- Insert a new row between existing rows without copy-paste.
- Turn a content row into a header row or demote it back.
- Reorder rows without copy-paste.

<Callout kind="info">

To quickly add a row to the end of the table, click the **+** button that appears below the last row.
