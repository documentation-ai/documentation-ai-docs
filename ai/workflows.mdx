...
Open the dashboard, then go to **Agent Settings** → **Workflows**. The Workflows area is where you create workflows, review previous runs, and inspect results.
...
<Callout kind="alert" collapsed="false">
  ...
  - A documentation site synced to a **connected Git repository** (GitHub or GitLab)

  - Any repository you want to use as a trigger repo or context repo connected in Documentation.AI

  - Optionally, connect [Jira](/integrations/jira) or [Confluence](/integrations/confluence) as additional context sources for the AI agent

  - Optionally, enable the **User & Agent Feedback** connector from Agent Settings to use the Self-Heal by User Feedback template
</Callout>
...
Templates are organized into two categories in the template picker:

- **Self-updating** templates generate or update documentation content based on code changes, repository activity, or user feedback.
- **Auditing** templates check and report on documentation health without modifying content directly.
...
<Columns cols="2">
  ...
  <Card title="Self-Heal by User Feedback" icon="message-square" horizontal="false">
    Analyze page ratings, user comments, and AI assistant confidence scores to find documentation gaps and propose fixes backed by evidence. Requires the User & Agent Feedback connector.
  </Card>
</Columns>
...
<Columns cols="2">
  <Card title="Broken Link Audit" icon="external-link" horizontal="false">
    Scan your published live site for broken internal links, anchors, redirects, and external URLs. Results include a grouped findings report with an AI-written summary. Use Chat with Agent on a finished run to open the findings in the editor chat and propose fixes.
  </Card>
...
<Steps>
  <Step title="Open the Workflows page" icon="home" title-type="p">
    In the dashboard, go to **Agent Settings** → **Workflows**.
  ...
  <Step title="Select connectors" icon="plug" title-type="p">
    The workflow form includes a connector selector that lets you include or exclude specific connectors for this workflow: GitHub, Jira, Confluence, and User Feedback.

    If a template requires a connector that is not yet enabled, the create button prompts you to enable it before proceeding.
  </Step>
...
<Callout kind="info" collapsed="false">
  In addition to repository context, the AI agent can also read from connected [Jira](/integrations/jira) issues and [Confluence](/integrations/confluence) pages during workflow runs. Connect them from **Settings → Integrations** and enable them as agent connectors for your documentation project.
</Callout>
...
- A **Chat with Agent** button that opens the editor chat pre-loaded with the run's findings, so you can ask follow-up questions or request fixes based on the results
...
<Callout kind="alert" collapsed="false">
  ...
  - Workflows require a documentation site synced to a **connected Git repository** (GitHub or GitLab)
