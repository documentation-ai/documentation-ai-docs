...
- **Append a row**: Hover near the **bottom edge** of the table and click the **+** button that appears. You can also **drag** the button downward to add multiple rows at once.
- **Insert between rows**: Hover over a row to reveal the row handle (vertical-ellipsis icon), click it, and choose **Insert row above** or **Insert row below**.

<Callout kind="info">

You cannot insert a row above a header row. If you need a new header row, insert below the current header and then toggle header styling using the **H** control.
...
To add a row, insert a new line with pipe-separated values at the desired position within the table block. Markdown does not have a dedicated row-insertion syntax — the row order in your source file determines the display order.
