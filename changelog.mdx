...
<Update label="2026-06-30" description="PDF export, SVG and HTML Block components, SEO meta overrides, API playground server management, heading copy-link, and platform improvements" tags={["feature","improvement","fix"]}>
  ...
  Remove the "Powered by Documentation.AI" footer from your published docs with a self-serve white-label toggle. Available as a $25/month add-on for Standard plans and above, or included at no extra cost on Enterprise. Once purchased, toggle white-label mode on or off per documentation site from the dashboard. Changes take effect immediately on your published site without a redeploy.
  ...
  - **Plan Limit Increases**: Standard plan now includes 3 editor seats (up from 2) and 2 documentation sites (up from 1). Professional plan now includes 10 editor seats (up from 5).

  ## Bug Fixes

  - **IME Input Fix**: Fixed an issue where dead-key input sequences (such as accented characters on macOS) were not registered correctly in the AI Assistant chat widget.
