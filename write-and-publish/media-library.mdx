...
- Check that the file is a supported type and within your plan's size limit. See [Media storage limits](/account-and-billing/usage-limits#media-storage).
...
If you also need a video's poster URL, switch any page to Markdown mode and insert the video from the **Media** tab. The inserted snippet contains both the video URL and the poster URL. Copy them, then undo the insert.

### How to add an image, video, or file in MDX
...
<Image
  ...
  alt="Dashboard overview with the Media tab selected"
/>
```

If you want a video file to play in the page, set `render-type="video"` and include the poster:
...
Commit and push as usual. The file is already hosted, so it renders as soon as the build completes. For all props, see [Images](/components/images), [Videos and iframes](/components/videos-and-iframes), and [Files](/components/files).
...
If usage is above 80%, a warning also appears in the media library. If you go over the limit, free space by deleting files or upgrade the plan. Uploads and publishing pause until you are under the limit; sites you have already published keep serving. See [Media storage limits](/account-and-billing/usage-limits#media-storage).
...
- [Media storage limits](/account-and-billing/usage-limits#media-storage) for plan quotas and per-file sizes
