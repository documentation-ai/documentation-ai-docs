...
Define OpenAPI `servers` to control the base URLs used by generated API docs and the interactive playground:
...
Generated request examples use the first server in the spec as their starting base URL. The live playground reads the full `servers` array, lets readers choose among available servers, and renders inputs for server variables. Variable values default to `default`, then the first `enum` value, then an empty value when neither is available.

<Callout kind="tip" collapsed="false">

  Use absolute server URLs for hosted documentation. Relative server URLs such as `/v2` can be ambiguous outside your API's own domain, so readers should use the playground base URL override if they need to test a relative server against a specific host.

</Callout>

### Playground proxy behavior

Live playground requests are sent through Documentation.AI's server-side proxy. The proxy resolves and validates the target URL before forwarding the request, so hosts that are internal, private, local, or unresolved may fail even when they work from your own machine or private network.

Requests to `localhost`, private IP ranges, link-local addresses, `.local` hosts, `.internal` hosts, or DNS names the proxy cannot resolve are blocked or unreachable. Use a public staging URL, a secure tunnel, or the playground base URL override with a host that Documentation.AI can reach.

### Parameter examples

OpenAPI examples improve both generated documentation and the live playground. Use `example` for one default value or `examples` for multiple named values:
...
When a parameter has multiple examples, the playground displays an example picker next to the input. The picker fills the input with the selected example value; it does not restrict the parameter unless the schema also defines an `enum`.
...
- **Server-aware playground**: Live endpoint testing based on the OpenAPI `servers` array
...
The playground also reads content-based parameters, where OpenAPI stores the schema and examples under `content` instead of a top-level `schema`. When both are present, top-level parameter fields take precedence.

<Callout kind="info" collapsed="false">
  For the clearest generated static parameter docs, include a top-level `schema` on each parameter. The playground can read the first media type under `content`, but generated type badges and examples are most reliable when the parameter schema is explicit.
</Callout>
